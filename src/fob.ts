/**
 * game/mode/fob.ts — freely-placeable Forward Operating Bases.
 *
...
import { TransitionState } from "../core/transition-state";

/** Materials required to place one FOB (see brief's economy: Logistics Cargo Delivery → +500 FOB Materials). */
const FOB_MATERIAL_COST = 500;
...
/**
 * Builds one FOB at `position` for `team`. Caller (economy layer) is responsible for validating
 * and deducting `FOB_MATERIAL_COST` in materials before calling this — this function only builds.
 */
...
export { placeFob, teardownFob, activeFobsByTeam, FOB_MATERIAL_COST };
