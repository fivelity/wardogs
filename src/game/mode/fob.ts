...
import { FOB_MATERIAL_COST } from "./economy";
...
function placeFob(
  ...
  for (let i = 0; i < FOB_SANDBAG_RING_COUNT; i++) {
    ...
    const bag = mod.SpawnObject(
      RuntimeSpawn_Common.SandBags_01_C90_A,
      offset,
      facing,
    ) as mod.Object;
    sandbags.push(bag);
...
function tryPlaceFob(
  ...
  if (!afforded) {
    mod.DisplayNotificationMessage(
      mod.Message("Not enough materials to build a FOB."),
      player,
    );
    return undefined;
  }
...
PortalGadget.onFireStart(async (player, _isZooming, getTarget) => {
  ...
  const facing = mod.GetSoldierState(
    player,
