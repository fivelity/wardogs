 ...
 * separate hotzone.ts occupancy tracker yet) because the HotZone's contribution is defined in
 * the brief purely as a presence-weight multiplier feeding ControlZone's majority calculation,
 * not a separate ticket track. `game/mode/hotzone.ts` (not yet built — see BUILD_GUIDE.md) will
 * own the flag's drift movement; it should import `isPlayerInsideHotZone`/`isPlayerInsideControlZone`
 * from here rather than re-subscribing to the same AreaTrigger events, per AGENTS.md §8's guidance
 * on not duplicating zone-occupancy bookkeeping.
 *
